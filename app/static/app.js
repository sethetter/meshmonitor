...
class MeshtasticMonitor {
    ...
    async loadStats() {
        try {
            ...
            if (data.success) {
                ...
                document.getElementById('messagesWithLocation').textContent = data.stats.messages_with_location;
            }
        } catch (error) {
    ...
    updateNodeFilter(nodes) {
        ...
        select.innerHTML = '<option value="">All Nodes</option>' +
            nodes.map(node => `<option value="${node.node_id}">${node.node_id}</option>`).join('');
            ...
            return `
                <div class="message-item ${locationClass}">
                    <div class="message-header">
                        <span class="message-from">${msg.from_node}</span>
                    ...
                    <div class="message-meta">
                        ${msg.to_node ? `<span>To: ${msg.to_node}</span>` : ''}
    ...
    updateMapMarkers(messages) {
        ...
        messages.forEach(msg => {
            if (msg.latitude && msg.longitude) {
                ...
                const popupContent = `
                    <strong>${msg.from_node}</strong><br>
